def evaluate_game_state(board, rows=6, cols=7, player=2):
    ...
    weights = {2: 0.1, 3: 0.3, 4: 1.0}  
    
    # Helper function to check sequences 
    def count_sequence(start, delta_row, delta_col):
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if game_state[index] == player:
                ...
                score += count_sequence(index, 0, 1)  # Horizontal
                score += count_sequence(index, 1, 0)  # Vertical
                score += count_sequence(index, 1, 1)  # Diagonal-down-right
                score += count_sequence(index, -1, 1)  # Diagonal-up-right
